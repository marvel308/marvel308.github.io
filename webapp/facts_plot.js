...
function plotFactsDataLinePlot() {
	function getChartVarsFromData(data) {
		...
		data.forEach(matchpoint => {
			...
			if (matchpoint[5] != 'Own Goal' && matchpoint[5] != 'Penalty Kick') {
...
function addTableForTeamFacts(team, data, title) {
...
function addHistogramForTeamFacts(team, data, title) {
	function getChartVarsFromData() {
		result = {
			data: [],
			columnNames: []
...
function addStackedForTeamFacts(team, data, critical_data, title) {
	function getChartVarsFromData() {
		...
		data.forEach(datapoint => {
			if(datapoint[6] == team && datapoint[5] != 'Own Goal' && datapoint[5] != 'Penalty Kick') {
...
function addPieForTeamFacts(team, data, title) {
	function getChartVarsFromData() {
		result = {
			data: [],
			columnNames: []
...
async function crticalAnalysisForTeamFacts(team) {
	...
	addTableForTeamFacts(team, data, 'Critical Goals');
	addHistogramForTeamFacts(team, data, 'Critical Goals scored per time');
	addStackedForTeamFacts(team, data, critical_data, 'Goals over time');
	addPieForTeamFacts(team, data, 'Goals scored per time');
}
